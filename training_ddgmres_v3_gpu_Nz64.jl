...
function load_data(basedir, f, k, A, Δt)
    ...
    @show maximum(E_data), mean(E_data), minimum(E_data)
    @show maximum(ne_data), mean(ne_data), minimum(ne_data)
    @show maximum(δE_data), mean(δE_data), minimum(δE_data)
...
function load_dir(basedir, f, k, A, Δ)
    A_str = @sprintf "%1.0e" A
    A_str = replace.(A_str, s"0" => "")
    Δt_str = ""
    if Δt == 1e-1
...
all_x = vcat(all_ne, all_E, all_onehot)
