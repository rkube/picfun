...
s = ArgParseSettings()
    @add_arg_table s begin 
        ...
        "--lambda"
            help = "Orthogonality regularization"
            arg_type = Float32
            default = 0.0001f0
            required = true
...
model = Chain(Parallel(vcat, model_cnn, model_par), 
              ...
              Dropout(prob_da),
              Dense(Nz * num_aug, Nz * num_aug)) |> gpu
...
cfg = Dict("num_aug" => num_aug, "optim_fun" => optim_fun, "batch_size" => batch_size, 
           "activation" => optim_fun, "conv1_width" => conv1_width, "conv2_width" => conv2_width,
           "model_dir" => "/scratch/gpfs/rkube/picfun/DD_GMRES/nnv6_traiing_Nz64")
