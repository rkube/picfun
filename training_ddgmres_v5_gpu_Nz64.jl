...
basedir = "/home/rkube/gpfs/picfun/DD_GMRES/baseline_Nz64"
...
for sim_param ∈ sim_param_list
    ...
    #push!(E_list, E)
    #push!(δE_list, δE)
    push!(ne_list, hcat([circshift(ne, (i, 0)) for i in [0, 1, 3, 5, 7]]...))
    push!(E_list, hcat([circshift(E, (i, 0)) for i in [0, 1, 3, 5, 7]]...))
    push!(δE_list, hcat([circshift(δE, (i, 0)) for i in [0, 1, 3, 5, 7]]...))
...
model_cnn = Chain(Conv((conv1_width, 1), 2=>channels_1, activation), 
                  ...
                  #Conv((conv3_width, 1), channels_2=>channels_3, activation), 
                  #Conv((conv4_width, 1), channels_3=>channels_4, activation), 
                  #Conv((conv5_width, 1), channels_4=>channels_5, activation), 
                  Flux.flatten)
model_par = Chain(Dense(6, num_dense, activation), 
                  #Dropout(prob_da),
                  Dense(num_dense, num_dense, activation))

num_intermediate = channels_2 * (Nz - sum([2*(x÷2) for x in [conv1_width conv2_width]])) # conv3_width conv4_width conv5_width]]))
