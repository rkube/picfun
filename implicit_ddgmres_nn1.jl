...
for idx ∈ 1:num_ptl
    ...
    # ptlₑ₀[idx] = particle(x0 + A .* init_fn(k * x0), 0.0) 
    ptlₑ₀[idx] = particle(x0 + A .* (0.5 * cos(k * x0) + 0.5 * sin(k * x0)), 0.0) 
    fix_position!(ptlₑ₀[idx], zgrid.Lz)
end

input_fn_onehot1 = make_sim_onehot(cos, k, A, Δt)
input_fn_onehot2 = make_sim_onehot(sin, k, A, Δt)
input_fn_onehot = 0.5 * (input_fn_onehot1 + input_fn_onehot2)
@show input_fn_onehot
...
num_pca = 24
ml_model_fname = @sprintf "simple_MLP_num_pca_%02d.bson" num_pca
...
for nn in 1:Nt
    ...
    t = @elapsed begin
        while (E_converged == false)
            ...
            x_in = [nₑ; E; input_fn_onehot]
            U = reshape(model(x_in), (Nz, num_pca))
            # @show U
