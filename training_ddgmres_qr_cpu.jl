...
function my_inv(A::AbstractMatrix{T}) where T
    # Calcualte the inverse using SVD. This is pretty much pinv.
    # C A = 1 => C = A⁻¹
    USV = svd(transpose(A) * A)
    C = USV.V * Diagonal(T(1.0) ./ USV.S) * transpose(USV.U)
    return C
end

function ChainRulesCore.rrule(::typeof(my_inv), A)
    C = my_inv(A)

    function my_inv_pullback(C̄)
        grad = -transpose(C) * C̄ * transpose(C)
        return NoTangent(), grad
    end

    return C, my_inv_pullback
end
...
function proj_loss(x, fᵐ)
    ...
    for i ∈ 1:this_batch
        ...
        # f = V*x
        #f = V * my_inv(transpose(V) * V) * transpose(V) * fᵐ[:, i]
        ...
        Q, _ = qr(V)
        f = ((Q[:, 1:num_aug]' * fᵐ[:, i])' * Q[:, 1:num_aug]')'
