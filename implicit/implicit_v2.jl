...
while(E_converged == false)
    ...
    for ele ∈ 1:num_ptl
        ...
        while(ptl_converged == false)
            # Calculate x_p^{n+1/2}
            xp_n12 = 0.5 * (ptl_e[ele].pos + xp_guess)
            # Calculate v_p^{n+1}
    ...
    print("Iteration(E) $(num_it_E): norm(E_old) = $norm(E_initial), norm(E_new) = $norm(E_new)")
